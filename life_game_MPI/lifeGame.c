...
int main(int argc, char* argv[]){
	...
	double startTime, endTime;
		...
		if (rank == MASTER){		
			...
			// Create window
			window = SDL_CreateWindow( "Práctica 3 de PSD - JAVIER ORBIS PABLO GARCÍA ® - LIFE GAME",
										0, 0, worldWidth * CELL_SIZE, worldHeight * CELL_SIZE, SDL_WINDOW_SHOWN);			
			...
			masterInit(size - 1, worldWidth, worldHeight, distModeStatic, grainSize,totalIterations, window, renderer, autoMode);
			...
			workerInit(rank, worldWidth, worldHeight);
		}
		MPI_Finalize();
